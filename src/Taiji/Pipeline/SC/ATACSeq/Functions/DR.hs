...
lsi :: (Elem 'Gzip tags ~ 'True, SCATACSeqConfig config)
    => FilePath
    -> SCATACSeq S (a, File tags 'Other)
...
-- | Perform LSA.
lsi' :: Elem 'Gzip tags ~ 'True
    => Maybe FilePath    -- ^ temp dir
    -> FilePath    -- ^ LSA output
    -> File tags 'Other
    -> IO ()
lsi' dir lsaNpy input = withTemp dir $ \tmp -> do
      tfidf (input^.location) tmp
      shelly $ run_ "taiji-utils" ["reduce", "--method", "svd", T.pack tmp, T.pack lsaNpy]
{-# INLINE lsi' #-}
